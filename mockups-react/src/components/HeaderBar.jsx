...
export default function HeaderBar() {
  ...
  const handleToggle = (view) => {
    setCurrentView(view)
    // Store the current view in localStorage to persist across page reloads
  ...
  useEffect(() => {
    ...
    // Get the stored dashboard view or detect from URL
    const storedView = localStorage.getItem('dashboardView')
    const currentPath = window.location.pathname
    
    if (storedView) {
      // Use stored view if available
      setCurrentView(storedView)
    } else if (currentPath.includes('/dashboard/supplier') || currentPath.includes('/dashboard/msme') || currentPath.includes('/supplier/')) {
      setCurrentView('supplier')
      localStorage.setItem('dashboardView', 'supplier')
    } else {
      setCurrentView('farmer')
      localStorage.setItem('dashboardView', 'farmer')
    }
  }, [])

  // Listen for URL changes to update toggle state
  useEffect(() => {
    const handleLocationChange = () => {
      const currentPath = window.location.pathname
      if (currentPath.includes('/dashboard/supplier') || currentPath.includes('/dashboard/msme') || currentPath.includes('/supplier/')) {
        if (currentView !== 'supplier') {
          setCurrentView('supplier')
          localStorage.setItem('dashboardView', 'supplier')
        }
      } else if (currentPath.includes('/dashboard/farmer')) {
  ...
  const handleLogout = () => {
    localStorage.clear()
    // Clear the dashboard view preference on logout
          ...
          <div className="header-logo">
            ...
            <div className="header-logo-text">
              ...
              <p className="header-subtitle">
                {currentView === 'supplier' ? 'Supplier Dashboard' : 'Farmer Dashboard'}
              </p>
            </div>
          </div>

          {/* Right controls: Marketplace button + Profile + Toggle */}
          <div className="header-actions">
            {/* Only show marketplace and service buttons when not in supplier mode */}
            ...
            <div className="header-toggle-container">
              <DashboardToggle currentView={currentView} onToggle={handleToggle} />
            </div>
